...
```hcl
module "ecs-cron" {
    source = "github.com/pbs/terraform-ecs-cron-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/ecs-cron'
gh -R 'pbs/terraform-ecs-cron-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/ecs-cron'
rm -f release.tar.gz
```
...
```bash
git remote add -f ecs-cron git@github.com:pbs/terraform-ecs-cron-module.git
git subtree add --prefix terraform/modules/ecs-cron ecs-cron main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/ecs-cron ecs-cron $REF --squash
```
